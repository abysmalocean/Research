...
C-----------------------------------------------------------------------
    ...
    1 IF(IPARM(3).EQ.1) RATE(I)=-2.*RATE(I)
      RETURN
      END
...
c*-------------------------------------------------------------
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*4 VX
      DIMENSION X(30),DP(19),F(19),v(1),DISTB(1)
      COMMON/AERSL1/IPARM(10)
     ...
     $ 13.9,10.5,7.78,5.9,4.57,3.22,2.36,1.58,.714,.705/
      ...
      ALNORM(TOT,GMEAN,GLSIG,VX)=.3989*TOT*EXP(-.5*((REAL(VX)/GMEAN)/
     $ GLSIG)**2)/(VX*GLSIG)
      
      IF(IPARM(2).EQ.1) GO TO 112
      DO 111 I=1,NPTS
 111  DISTB(I)=ALNORM(DBLE(1.06E5),DBLE(1.44E-18),DBLE(1.76),REAL(V(I)))
     $ +ALNORM(DBLE(3.2E4),DBLE(8.24E-17),DBLE(2.31),REAL(V(I)))
     $ +ALNORM(DBLE(5.4),DBLE(3.33E-13),DBLE(2.38),REAL(V(I)))
      RETURN
 112  IF(ICALL.EQ.1) GO TO 122
      ICALL=1
      DO 123 J=1,19
      DP(J)=ALOG(REAL(F(J)))
 123  F(J)=ALOG(REAL(F(J)))
 122  DO 3 NP=1,NPTS
      D=(6.E12*V(NP)/3.14159)**(.33333333333)
      DL=ALOG(REAL(D))
      DO 124 K=2,19 
      IF(DL.LT.DP(K)) GO TO 125
 124  CONTINUE
C 125  DL=E(K)+(DP(K)-DL)/(DP(K)-DP(K-1))*(F(K-1)-F(K))
 125  DL=(DP(K)-DL)/(DP(K)-DP(K-1))*(F(K-1)-F(K))
